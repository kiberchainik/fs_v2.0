...
export class MailService {
    ...
    public async sendFeedBackEmail(email: string, name: string, emailFrom: string, message: string) {
        const html = await render(FeedbackTemplate({ emailFrom, name, message }))

        return this.sendMail(email, 'FS Feedback', html)
    }
